...
export default function LibraryDeskTop({
  ...
  const availableMap = useMemo(() => {
    ...
    for (const seat of table.seats) {
      const isTaken = reservationsStatuses
        ? reservationsStatuses.findIndex(r => r.seatId === seat.id && (r.status === "Active" || r.status === "Temp")) !== -1
        : false;
      map.set(seat.id, !isTaken);
    }
    return map;
  }, [table.seats, reservationsStatuses]);

  return (
    <div className="flex flex-col items-center w-full pt-4">
      <div className="flex flex-row gap-5 justify-center">
        {table.seats.map(seat => {
          const isAvailable = availableMap.get(seat.id) ?? true;
          return (
            <SeatIcon
              key={seat.id}
              ...
              onClick={() => {
                if (isAvailable || selectedSeatId === seat.id) {
                  onSelectSeat(seat.id, seat.seatNumber, table.name);
                }
              }}
