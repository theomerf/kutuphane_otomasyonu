...
export default function AuthorsAdmin() {
    ...
    const fetchAuthors = async (queryParams: RequestParameters, signal?: AbortSignal) => {
        try {
            ...
            const response = await requests.authors.getAllAuthors(queryString, signal);
    ...
    useEffect(() => {
        ...
        const loadAuthors = async () => {
            try {
                ...
                const authors = await fetchAuthors(finalQuery);
                setData(authors);
            }
            catch (error: any) {
                if (error.name === "CanceledError" || error.name === "AbortError") {
                    ...
                    setError("Yazarler yüklenirken bir hata oluştu.");
                }
            }
            finally {
                setIsLoading(false);
    ...
    return (
        <div className="flex flex-col">
            <div className="flex flex- row">
                <p className="font-semibold text-4xl ml-8 lg:ml-20 text-violet-500 h-fit border-none pb-2 mb-12 relative after:content-[''] after:absolute after:bottom-[-10px] after:left-0 after:w-20 after:h-1 after:bg-hero-gradient after:rounded-sm">Yazar Yönetimi</p>
                ...
                <Link to="/admin/authors/create" className="button !bg-green-400 hover:scale-105 text-lg font-bold duration-500 self-center ml-auto mr-20">
                            ...
                            <table className="table-auto w-full border-collapse border border-gray-200 bg-white shadow-lg">
                                <thead>
                                    <tr className="bg-violet-400">
                                        ...
                                        <th className="text-white font-bold text-lg border border-gray-200 px-4 py-6 text-center">Ana Yazar</th>
                                        ...
                                        <tr key={cat.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="border-l border-t border-b border-violet-200 px-4 py-6">
                                                <div className="flex flex-row justify-center gap-x-2">
                                                    <Link to={`/admin/authors/update/${cat.id}`} title="Düzenle" className="bg-yellow-500 rounded-lg text-center flex justify-center content-center align-middle text-white w-10 h-10 hover:scale-105 hover:bg-yellow-600 duration-500 text-lg">
                                                        <FontAwesomeIcon icon={faEdit} className="self-center" />
                                                    </Link>
                                                    <button onClick={() => handleAuthorDelete(cat.id!)} title="Sil" className="bg-red-500 rounded-lg text-center flex justify-center content-center align-middle text-white w-10 h-10 hover:scale-105 hover:bg-red-600 duration-500 text-lg">
