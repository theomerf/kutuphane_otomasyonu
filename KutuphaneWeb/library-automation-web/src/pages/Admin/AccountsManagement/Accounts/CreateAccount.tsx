            ...
            <form method="POST" onSubmit={handleSubmit(handleAccountCreation)} noValidate>
                <div className={`${isMobile ? 'py-6' : 'py-10'} text-center bg-violet-500 rounded-tl-lg rounded-tr-lg`}>
                    <p className={`text-white font-bold ${isMobile ? 'text-xl' : 'text-2xl md:text-3xl'}`}>
                        ...
                        {isMobile ? 'Yeni Kullanıcı' : 'Yeni Kullanıcı Ekle'}
                    </p>
                </div>
                <div className={`flex flex-col ${isMobile ? 'gap-y-4 px-4 py-6' : 'gap-y-6 px-8 py-10'} rounded-lg shadow-xl bg-white border border-gray-200`}>
                    ...
                    <div className={`flex ${isMobile ? 'flex-col gap-y-4' : 'flex-row gap-x-10'}`}>
                        ...
                        <div className={`flex flex-col ${isMobile ? 'w-full' : 'w-1/2'}`}>
                            <label htmlFor="password" className={`font-bold text-gray-500 ${isMobile ? 'text-sm' : 'text-base'}`}>Şifre</label>
                            <div className={`flex ${isMobile ? 'mt-2' : 'mt-4'}`}>
                            ...
                            {errors.password && <span className="text-red-700 text-xs md:text-sm text-left mt-1">{errors.password?.message?.toString()}</span>}
                        </div>
                    </div>
